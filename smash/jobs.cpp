...
int Jobs_list::add_job(const string cmd, pid_t son_pid) {
    this->garbage_collector();
    if (this->jobs_list.size() < MAX_JOBS) {
        Job new_job = Job(cmd, son_pid);
...
int Jobs_list::get_min_job_id() {
    ...
    for (int id = 1; id <= MAX_JOBS; id++) {
        auto it = this->jobs_list.find(id);
        if (it == this->jobs_list.end()) return id;
    }
    return 0;
}
...
int Jobs_list::print_job(int job_id) {
    ...
    // Get job data
    string cmd_string = this->jobs_list.at(job_id).cmd;
